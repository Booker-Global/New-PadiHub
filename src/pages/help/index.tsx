...
const SHOW_BROWSE_BY_TOPIC = false;
const SHOW_POPULAR_ARTICLES = false;
const SHOW_VIDEO_GUIDES = false;
  ...
  return (
    <HelpRouteLayout>
      <Helmet>
        ...
        <meta name="description" content="Get help with PadiHub — search our FAQs, submit a ticket or chat with our team." />
        <link rel="canonical" href="https://www.padihub.com/help" />
        <meta property="og:title" content="Help & Support — PadiHub" />
        <meta property="og:description" content="Get help with PadiHub — search our FAQs, submit a ticket or chat with our team." />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.75)' }}>
              Search our FAQs, start a live chat, or submit a ticket for help.
            </p>
        ...
        <MotionDiv variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {channels.filter(channel => !channel.hidden).map(channel => (
            <MotionDiv
              key={channel.label}
              variants={fadeUp}
              className="bg-white rounded-2xl p-4 text-center shadow-sm border border-gray-100 hover:shadow-md transition-all group"
            >
              <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3 transition-transform group-hover:scale-110" style={{ background: `${channel.color}15` }}>
          ...
          <MotionDiv variants={fadeUp} initial="hidden" animate="visible">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {categories.map(category => (
                ...
                >
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110" style={{ background: `${category.color}15` }}>
                    <category.icon size={16} style={{ color: category.color }} />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-gray-900">{category.label}</p>
              ...
              {popularArticles.map(article => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{article.category} · {article.reads.toLocaleString()} reads</p>
                  </div>
        ...
        <MotionDiv variants={fadeUp} initial="hidden" animate="visible">
            ...
            <div>
              ...
              {normalizedSearch && (
                <p className="text-xs text-gray-500 mt-1">
                  {faqResults.length} result{faqResults.length === 1 ? '' : 's'} for “{search.trim()}”
                </p>
              )}
            </div>
            <div className="flex items-center gap-3">
              <Link to="/faq" className="text-xs font-bold transition-colors hover:opacity-80" style={{ color: '#8B5CF6' }}>
                View full FAQ
              </Link>
              <Link to="/help/ticket" className="text-xs font-bold flex items-center gap-1 transition-colors hover:opacity-80" style={{ color: '#2EAF6F' }}>
          ...
          {faqResults.length ? (
            <div className="space-y-2">
              {faqResults.map(faq => (
                <div key={faq.q} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  <button
                    onClick={() => setOpenFaq(openFaq === faq.q ? null : faq.q)}
                    className="w-full flex items-center justify-between p-4 text-left hover:bg-gray-50 transition-colors"
                  >
                    <p className="text-sm font-bold text-gray-900 pr-4">{faq.q}</p>
                    <MotionDiv animate={{ rotate: openFaq === faq.q ? 180 : 0 }} transition={{ duration: 0.2 }} className="flex-shrink-0">
                      <ChevronDown size={16} style={{ color: '#9CA3AF' }} />
                    </MotionDiv>
                  </button>
                  {openFaq === faq.q && (
                    <MotionDiv initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="px-4 pb-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 text-center">
              <p className="text-sm font-bold text-gray-900 mb-1">No FAQ matches found</p>
              <p className="text-sm text-gray-500 mb-4">Try another search, browse the full FAQ page, or submit a ticket.</p>
              <div className="flex items-center justify-center gap-3">
                <Link to="/faq" className="text-sm font-bold" style={{ color: '#8B5CF6' }}>Browse FAQs</Link>
                <Link to="/help/ticket" className="text-sm font-bold" style={{ color: '#2EAF6F' }}>Submit a ticket</Link>
              </div>
            </div>
          )}
        </MotionDiv>
