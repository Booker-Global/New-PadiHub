...
export default function CookieBanner() {
  ...
  // Both isEmbedded and hideForBuilderPreview must start as false (the SSR-safe
  // default) and only resolve their real values inside useEffect. Reading
  // window.parent or sessionStorage during render causes a server/client mismatch
  // because the server always sees window as undefined, producing a different
  // initial render than the client — which triggers hydration error #418.
  const [hideForBuilderPreview, setHideForBuilderPreview] = useState(false);

  useEffect(function checkConsent() {
    if (typeof window === 'undefined') return;

    // Resolve isEmbedded and hideForBuilderPreview here — safe because this
    // runs only in the browser, after hydration is complete.
    const isEmbedded = window.parent !== window;
    if (isEmbedded) {
      try {
        const shouldHide = sessionStorage.getItem(BANNER_RESET_KEY) !== 'true';
        setHideForBuilderPreview(shouldHide);
      } catch (e) {
